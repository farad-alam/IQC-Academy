...
export default function FinalExamClient({ course }) {
  const router = useRouter();
  const [quizzes, setQuizzes] = useState(course.finalExamQuizzes || []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If we need to edit later we could add a PATCH route, for now let's assume we might just recreate or if we have patch
      // Since we didn't add PATCH route for final exam quizzes, let's just make it create. If we need edit, we'd add PATCH.
      // But let's check if isEditing is true
      const isEditing = !!editingQuizId;
      const url = isEditing
        ? `/api/admin/courses/${course.id}/final-exam/${editingQuizId}`
        : `/api/admin/courses/${course.id}/final-exam`;
  ...
  const handleDelete = async (quizId) => {
    ...
    try {
      const res = await fetch(`/api/admin/courses/${course.id}/final-exam/${quizId}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  return (
    <div>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <Link href={`/admin/courses/${course.id}/modules`} className="btn btn-ghost btn-sm" style={{ padding: 0, marginBottom: '0.5rem' }}>
            <ChevronLeft size={16} /> মডিউলে ফিরে যান
          </Link>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 700, marginBottom: '0.25rem' }}>ফাইনাল পরীক্ষা কুইজ</h1>
          <p style={{ color: 'var(--color-text-muted)' }}>কোর্স: {course.title}</p>
        </div>
        {!isAdding && (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        {quizzes.length === 0 && (
          <div className="card" style={{ padding: '3rem', textAlign: 'center', color: 'var(--color-text-muted)' }}>
            এই কোর্সের ফাইনাল পরীক্ষার কোনো কুইজ নেই।
          </div>
        )}
      </div>
    </div>
