      ...
      <div className="container" style={{ padding: '2rem 1rem', maxWidth: '600px' }}>
        ...
        {history && history.length > 0 && (
          <div className="card" style={{ padding: '2rem', marginTop: '2rem' }}>
            <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '1rem' }}>আগের ফলাফলসমূহ:</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {history.map((h, i) => (
                <div key={h.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', backgroundColor: 'var(--color-surface-alt)', borderRadius: '8px' }}>
                  ...
                  <div style={{ fontSize: '0.875rem', color: 'var(--color-text-muted)' }}>
                    {new Date(h.takenAt).toLocaleDateString('bn-BD')}
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="container" style={{ padding: '2rem 1rem', maxWidth: '600px' }}>
        <div className="card" style={{ padding: '3rem 2rem', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', flexDirection: 'column' }}>
            ...
            <Link href={`/learn/${module.subject.course.id}`} className={passed && !resultData?.nextModuleId ? "btn btn-primary" : (passed ? "btn btn-outline" : "btn btn-primary")}>
              কোর্সে ফিরে যান
            </Link>
          </div>
        </div>
